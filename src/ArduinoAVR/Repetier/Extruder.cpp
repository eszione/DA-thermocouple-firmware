...
#ifdef SUPPORT_MAX6675
    case 101: // MAX6675
        // Call a MAX6675 more often than every 170ms and it will never change its reported temperature reading
        if (counter250ms<=1)
...
#ifdef SUPPORT_MAX6675
#ifdef BITBANG_MAX6675
// Replacement thermocouple code using bit-banging hardware SPI
// vik@diamondage.co.nz, after Ryan McLaughlin <ryanjmclaughlin@gmail.com>
int16_t read_max6675(uint8_t ss_pin)
{
    int16_t max6675_temp = 0;
    int16_t retries;
    int16_t error_tc;
    // Allow 3 retries to cope with noisy thermocouples.
    for (retries=0; retries<3; retries++) {
      // Manually init the SPI port pins.
      HAL::pinMode(MISO_BITBANG_PIN, INPUT);
      HAL::pinMode(SCK_BITBANG_PIN, OUTPUT);
 
      HAL::pinMode(ss_pin, OUTPUT);
      HAL::digitalWrite(ss_pin,HIGH);  // Disable device
 
      HAL::digitalWrite(ss_pin, 0);  // enable TT_MAX6675
      /* Cycle the clock for dummy bit 15 */
      HAL::digitalWrite(SCK_BITBANG_PIN,HIGH);
      HAL::digitalWrite(SCK_BITBANG_PIN,LOW);
      ...
      */
      for (int16_t i=11; i>=0; i--){
        HAL::digitalWrite(SCK_BITBANG_PIN,HIGH);  // Set Clock to HIGH
        max6675_temp += HAL::digitalRead(MISO_BITBANG_PIN) << i;  // Read data and add it to our variable
        HAL::digitalWrite(SCK_BITBANG_PIN,LOW);  // Set Clock to LOW
      }
    
      /* Read the TC Input inp to check for TC Errors */
      HAL::digitalWrite(SCK_BITBANG_PIN,HIGH); // Set Clock to HIGH
      error_tc = HAL::digitalRead(MISO_BITBANG_PIN); // Read data
      HAL::digitalWrite(SCK_BITBANG_PIN,LOW);  // Set Clock to LOW

      HAL::digitalWrite(ss_pin, 1);  // disable TT_MAX6675 
      // Check error bit to see if thermocouple is open or noisy.
      // Also trap zero readings, which indicate no MAX6675!
      if ((error_tc == 0) && (max6675_temp!=0))
        // Return the raw temp reading as C*4
        return max6675_temp;
    }
    // Too many retries, return large number
    return 2000;
}
#else  // BITBANG_MAX6675
int16_t read_max6675(uint8_t ss_pin)
{
    int16_t max6675_temp = 0;
    int16_t retries;
    ...
    for (retries=0; retries<3; retries++) {
      ...
      // Check error bit to see if thermocouple is open or noisy.
      if ((max6675_temp & 4) == 0)
        // Drop last 3 bits and return the raw temp reading.
        return max6675_temp >> 3;
    }
    // Too many retries, return large number
    return 2000;
}
#endif  // BITBANG_MAX6675
#endif
#ifdef SUPPORT_MAX31855
